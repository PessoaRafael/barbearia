-- Reservar passando a respeitar o plano do assinante.
--
-- Duas travas novas, e as duas moram aqui e não na tela: a categoria do
-- serviço tem que estar no plano, e o dia da semana também. A tela esconde o
-- botão do clube quando não cabe, mas quem recusa de verdade é esta função,
-- porque o chat e um POST na mão chegam pelo mesmo caminho.
--
-- Sexta e sábado não bloqueiam o agendamento: só tiram o benefício. O cliente
-- marca igual e paga o preço cheio.
...
begin
  ...
  if p_usar_clube then
    ...
    -- O serviço precisa estar na categoria do plano E marcado como coberto.
    -- O plano diz o que o pacote inclui; a coluna do serviço é a exceção que o
    -- Johny controla no painel, para tirar um item caro de dentro do pacote.
    ...
    if not (v_dow = any (v_plano.dias_semana)) then
